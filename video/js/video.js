    ...
    request1.success(function(videoes) {
        ...
        if(videoes.item){
            ...
            $.each(videoes, function(index,video){
                VideoHtml += '<div id="'+ video.id  +'"' +
                    ' onclick=' + 'videoEvent("' + video.id + '")'  +
                    ' class="col-xs-6 col-sm-3 content-cell video-type-content" item="' + mid + '" name="' + video.title + '">' +
                    '<div class="video-img">' +
...
function videoEvent(videoId) {
    ...
    var resType = id;
    if(mid == "0404"){
        window.location.href="musicDetails.html?mid="+ mid + "&item="+item+"&id="+id+"&resType="+curIndex+"&ids="+Math.random()+"&phone="+getPhoneUser();
		//window.location.href="AudioController.php";
    }
    else if(mid == "0500")
    {
        window.location.href = namePage + "View.html?res_id=" + id + "&name_page=" + curIndex+"&phone="+getPhoneUser();
    }
    else{
        window.location.href="videoDetails.html?mid="+ mid + "&item="+item+"&id="+id+"&resType="+resType+"&ids="+Math.random()+"&phone="+getPhoneUser();
    }
};
