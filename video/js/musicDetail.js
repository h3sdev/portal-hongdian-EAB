...
function initiaList(mid,itemId,docid,curIndex) {
    ...
    request.success(function(data) {//得到音乐数据
        ...
        // var MusicTitle = '<div class="header">' + audioPlayer.playlist[curPlayIndex].title + '</div>';
        ...
        //歌词页 Comentado para no mostrar LRC 
...
function setInfo(currPlayer) {
    ...
    //loadImage(img_path);
    // var MusicTitle = '<div class="header" title="' + currPlayer.title + '">' + 'Música' + '</div>';
    // $(".header-center").html(MusicTitle);
...
function playNext() {
    ...
    if(playType == 'order'){//顺序播放 Reproducción secuencial
